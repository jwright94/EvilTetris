...
def drawTimer(text):
	titleSurf, titleRect = makeTextObjs(text, BIGFONT, TEXTSHADOWCOLOR)
	titleRect.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT) - 100)
	DISPLAYSURF.blit(titleSurf, titleRect)
...
def onQuiz():
	...
	#invert(surface, WINDOWWIDTH, WINDOWHEIGHT)
	#DISPLAYSURF.blit(surface, (0, 0))
	
	question = quiz.getQuestion()
	keys = random.sample(quizKeys, len(question.options))
	
	#titleSurf, titleRect = makeTextObjs(question.question, BASICFONT, TEXTSHADOWCOLOR)
	#titleRect.topleft = (60, 60)
	
	surfsAndRects = []
	
	for i in range(len(keys)):
		line = keys[i][1] + ') ' + question.options[i]
		s, r = makeTextObjs(line, BASICFONT, (255, 0, 0))
		r.topleft = (60, 60 + 60 + 64*i)
		surfsAndRects.append((s, r))
	...
	while True:
		...
		drawEverything()
		checkForQuit()
		for event in pygame.event.get([KEYDOWN, KEYUP]): # event handling loop
			if event.type == KEYDOWN:
				for i in range(len(keys)):
					if event.key == keys[i][0]:
						success = i == question.answer
						if not success:
							random.choice(failSounds).play()
							flipboard()
						else:
							random.choice(successSounds).play()
						return success
		
		
		
		#DISPLAYSURF.blit(titleSurf, titleRect)
		blit_text(DISPLAYSURF, question.question, (60, 60), BASICFONT, WHITE)
		for x in surfsAndRects:
			DISPLAYSURF.blit(x[0], x[1])
		drawTimer("0:" + str(int(timeleft)).zfill(2))
		pygame.display.flip()
		pygame.time.wait(1000/30)
		timeleft -= 1000.0/30.0/1000.0 # close enough
		if timeleft <= 0:
			random.choice(failSounds).play()
			flipboard()
			return False;
